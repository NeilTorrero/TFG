...
class AudioDataLayer(IterableDataset):
    ...
    def set_signal(self, signal):
        self.signal = signal.astype(numpy.float32) / 32768.
...
class FrameASR:
    ...
    def __init__(self, model_definition,
        ...
        self.buffer = numpy.zeros(shape=2 * self.n_frame_overlap + self.n_frame_len,
                                  dtype=numpy.float32)
        self.offset = offset
        self.reset()
    ...
    def transcribe(self, frame=None, merge=True):
        if frame is None:
            frame = numpy.zeros(shape=self.n_frame_len, dtype=numpy.float32)
        if len(frame) < self.n_frame_len:
            frame = numpy.pad(frame, [0, self.n_frame_len - len(frame)], 'constant')
    ...
    def reset(self):
        ...
        '''
        self.buffer = numpy.zeros(shape=self.buffer.shape, dtype=numpy.float32)
        self.prev_char = ''
...
if len(input_devices):
    ...
    def callback(in_data, frame_count, time_info, status):
        global empty_counter, crnt_sentence, stt
